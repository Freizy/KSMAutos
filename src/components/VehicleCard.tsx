...
export const VehicleCard: React.FC<VehicleCardProps> = ({ vehicle, onInquire, onCompare, onViewDetails, isComparing }) => {
  return (
    <motion.div
      ...
      className="group relative flex flex-col bg-white/5 border border-line rounded-sm overflow-hidden hover:border-accent transition-all duration-500 cursor-pointer"
    >
      {/* Image Container */}
      <div className="relative aspect-[16/10] overflow-hidden">
        ...
        <div className="absolute top-4 left-4 flex gap-2">
          <span className={cn(
            ...
            vehicle.status === 'sold' ? "bg-red-500/20 text-red-400 border border-red-500/30" :
      ...
      <div className="p-6 flex flex-col gap-4">
        <div className="flex flex-col gap-1">
          <div className="flex flex-col">
            <span className="text-[10px] text-muted uppercase tracking-widest font-bold">{vehicle.year} {vehicle.make}</span>
            <h3 className="text-2xl font-black tracking-tighter uppercase leading-none group-hover:text-accent transition-colors">{vehicle.model}</h3>
          </div>
          <div className="text-xl font-mono font-bold tracking-tighter text-accent mt-1">
        ...
        <div className="grid grid-cols-3 gap-4 py-4 border-y border-line">
          <Spec icon={<Gauge className="w-3 h-3" />} label="Top Speed" value={`${vehicle.topSpeed} KM/H`} />
          <Spec icon={<Zap className="w-3 h-3" />} label="0-100" value={`${vehicle.acceleration}S`} />
          <Spec icon={<Activity className="w-3 h-3" />} label="Power" value={`${vehicle.horsepower} BHP`} />
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <button 
            ...
            className="flex items-center justify-between opacity-50 hover:opacity-100 transition-opacity w-full group/details"
          >
            <span className="text-[8px] font-bold uppercase tracking-widest group-hover/details:text-accent transition-colors">Technical Data</span>
            <ChevronRight className="w-3 h-3 group-hover/details:translate-x-1 transition-transform" />
          </button>
        </div>
      </div>
    </motion.div>
