...
export default function Garage({
  ...
  const [activeGarageTab, setActiveGarageTab] = useState<
    "collection" | "wishlist"
  >("collection");
  ...
  const wishlistVehicles = inventory.filter((v) => wishlist.includes(v.id));
  ...
  return (
    <div className="flex flex-col gap-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-6">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-muted text-sm max-w-md mt-2">
            Manage your high-performance collection, track technical
            specifications, and monitor market valuation.
          </p>
        </div>
      ...
      <div className="flex border-b border-line">
        <button
          onClick={() => setActiveGarageTab("collection")}
          className={cn(
            "px-8 py-4 text-xs font-bold uppercase tracking-widest transition-all border-b-2",
            activeGarageTab === "collection"
              ? "border-accent text-accent"
              : "border-transparent text-muted hover:text-ink",
          )}
        >
          My Collection ({vehicles.length})
        </button>
        <button
          onClick={() => setActiveGarageTab("wishlist")}
          className={cn(
            "px-8 py-4 text-xs font-bold uppercase tracking-widest transition-all border-b-2",
            activeGarageTab === "wishlist"
              ? "border-accent text-accent"
              : "border-transparent text-muted hover:text-ink",
          )}
        >
          Wishlist ({wishlistVehicles.length})
        </button>
      </div>
      ...
      {activeGarageTab === "collection" && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <StatCard
            ...
            value={`$${vehicles.reduce((acc, v) => acc + (v.price || 0), 0).toLocaleString()}`}
            icon={<TrendingUpIcon />}
          />
          <StatCard
            label="Total Horsepower"
            value={`${vehicles.reduce((acc, v) => acc + (v.horsepower || 0), 0).toLocaleString()} BHP`}
            icon={<ZapIcon />}
          />
          <StatCard
      ...
      {activeGarageTab === "collection" ? (
        vehicles.length > 0 ? (
          ...
          <div className="py-24 glass flex flex-col items-center justify-center text-center gap-6 border-dashed">
            ...
            <div>
              ...
              <p className="text-muted text-sm max-w-xs mx-auto mt-2">
                You haven't added any vehicles to your collection yet. Start by
                adding your first high-performance machine.
              </p>
            </div>
            <button
              onClick={() => setShowAddModal(true)}
        ...
        <div className="py-24 glass flex flex-col items-center justify-center text-center gap-6 border-dashed">
          <div className="w-20 h-20 bg-red-500/10 flex items-center justify-center rounded-full">
          ...
          <div>
            ...
            <p className="text-muted text-sm max-w-xs mx-auto mt-2">
              Your wishlist is currently empty. Explore the showroom and add
            ...
            >
              ...
              >
                ...
                <div className="md:col-span-2 mt-4">
                  <button
                    type="submit"
                    className="w-full py-4 bg-accent text-bg font-bold uppercase tracking-widest rounded-sm hover:scale-[1.02] active:scale-[0.98] transition-all"
                  >
