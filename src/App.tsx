...
export default function App() {
  ...
  if (settings?.maintenanceMode && profile?.role !== 'admin' && user?.email?.toLowerCase() !== 'samenso100@gmail.com') {
    return (
      <div className="min-h-screen bg-bg flex items-center justify-center p-8 text-center">
        <div className="max-w-md flex flex-col items-center gap-6">
          ...
          <h1 className="text-4xl font-black tracking-tighter uppercase">Maintenance Mode</h1>
          <p className="text-muted text-sm uppercase tracking-widest font-bold">The KSM Autos showroom is currently undergoing technical upgrades. Please check back shortly.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-bg text-ink selection:bg-accent selection:text-bg overflow-x-hidden">
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="hidden md:flex items-center gap-1">
              ...
              <NavButton 
                ...
                label="My Garage"
              />
              { (profile?.role === 'admin' || user?.email?.toLowerCase() === 'samenso100@gmail.com') && (
                <NavButton 
      ...
      <main className="pt-16 min-h-screen">
        <AnimatePresence mode="wait">
              ...
              <div ref={showroomRef} className="max-w-7xl mx-auto px-4 py-24 border-b border-line">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {inventory.filter(v => v.isFeatured).slice(0, 2).map(v => (
                    <div key={v.id} className="relative aspect-[16/9] overflow-hidden group cursor-pointer" onClick={() => handleViewDetails(v)}>
          ...
          {activeTab === 'admin' && user && (profile?.role === 'admin' || user?.email?.toLowerCase() === 'samenso100@gmail.com') && (
            <motion.div
              key="admin"
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
        ...
        {showDetailsModal && selectedVehicle && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <button 
                onClick={() => setShowDetailsModal(false)}
                className="absolute top-4 right-4 z-10 p-2 bg-bg/60 backdrop-blur-md rounded-full text-muted hover:text-ink transition-colors"
              >
                ×
              </button>

              <div className="w-full md:w-1/2 h-64 md:h-auto overflow-hidden relative group/gallery">
                <AnimatePresence mode="wait">
                  <motion.img 
                    ...
                    src={selectedVehicle.images?.[activeImageIndex] || selectedVehicle.imageUrl} 
                    alt="" 
                ...
                {selectedVehicle.images && selectedVehicle.images.length > 1 && (
                  <>
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        setActiveImageIndex(prev => (prev === 0 ? selectedVehicle.images!.length - 1 : prev - 1));
                      }}
                      className="absolute left-4 top-1/2 -translate-y-1/2 p-2 bg-bg/60 backdrop-blur-md rounded-full text-muted hover:text-accent transition-all md:opacity-0 group-hover/gallery:opacity-100"
                    >
                    ...
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        setActiveImageIndex(prev => (prev === selectedVehicle.images!.length - 1 ? 0 : prev + 1));
                      }}
                      className="absolute right-4 top-1/2 -translate-y-1/2 p-2 bg-bg/60 backdrop-blur-md rounded-full text-muted hover:text-accent transition-all md:opacity-0 group-hover/gallery:opacity-100"
                    >
                      ...
                      {selectedVehicle.images.map((_, idx) => (
                        <button 
                          ...
                          className={cn(
                            ...
                            activeImageIndex === idx ? "bg-accent w-4" : "bg-white/30 hover:bg-white/50"
                          )}
                        />
                      ))}
                    </div>
              ...
              <div className="flex-1 p-8 flex flex-col gap-8 overflow-y-auto">
                <div>
                  <div className="text-xs text-accent uppercase tracking-widest font-bold mb-1">{selectedVehicle.year} {selectedVehicle.make}</div>
                  <h2 className="text-5xl font-black tracking-tighter uppercase leading-none">{selectedVehicle.model}</h2>
                  <div className="text-2xl font-mono font-bold text-accent mt-2">${selectedVehicle.price?.toLocaleString()}</div>
                </div>

                <div className="grid grid-cols-2 gap-6 py-8 border-y border-line">
                  <DetailSpec icon={<Gauge className="w-4 h-4" />} label="Top Speed" value={`${selectedVehicle.topSpeed} KM/H`} />
                  <DetailSpec icon={<Zap className="w-4 h-4" />} label="0-100 KM/H" value={`${selectedVehicle.acceleration}S`} />
                  <DetailSpec icon={<Activity className="w-4 h-4" />} label="Horsepower" value={`${selectedVehicle.horsepower} BHP`} />
                  <DetailSpec icon={<Car className="w-4 h-4" />} label="Category" value={selectedVehicle.category} />
                </div>

                <div className="flex flex-col gap-4">
                  <h4 className="text-[10px] uppercase tracking-widest font-bold text-muted">Description</h4>
                  <p className="text-sm text-muted leading-relaxed">
                    {selectedVehicle.description || "Experience the pinnacle of automotive engineering with this exceptional high-performance vehicle. Meticulously crafted for elite drivers who demand uncompromising speed and luxury."}
                  </p>
                </div>

                {selectedVehicle.features && (
                  <div className="flex flex-col gap-4">
                    <h4 className="text-[10px] uppercase tracking-widest font-bold text-muted">Key Features</h4>
                    <div className="flex flex-wrap gap-2">
                      {selectedVehicle.features.map(feature => (
                        <span key={feature} className="px-3 py-1 bg-white/5 border border-line rounded-full text-[10px] font-bold uppercase tracking-widest">
                          {feature}
                        </span>
                      ))}
                    </div>
...
function DetailSpec({ icon, label, value }: { icon: React.ReactNode, label: string, value: string }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        <div className="text-[8px] uppercase tracking-widest font-bold text-muted">{label}</div>
